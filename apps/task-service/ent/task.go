...
func (*Task) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case task.ForeignKeys[0]: // tag_tasks
			values[i] = new(sql.NullInt64)
...
func (t *Task) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case task.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field tag_tasks", value)
			} else if value.Valid {
				t.tag_tasks = new(int)
				*t.tag_tasks = int(value.Int64)
			}
		case task.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field user_tasks", values[i])
			} else if value.Valid {
				t.user_tasks = new(string)
				*t.user_tasks = value.String
			}
		}
	}
	return nil
