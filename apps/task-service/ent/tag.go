...
func (*Tag) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case tag.FieldSlug, tag.FieldDescription, tag.FieldColor:
			values[i] = new(sql.NullString)
		case tag.FieldCreatedAt, tag.FieldUpdatedAt:
			values[i] = new(sql.NullTime)
		case tag.ForeignKeys[0]: // tag_owner
			values[i] = new(sql.NullString)
		case tag.ForeignKeys[1]: // user_tags
			values[i] = new(sql.NullString)
		default:
...
func (t *Tag) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case tag.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field tag_owner", values[i])
			} else if value.Valid {
				t.tag_owner = new(string)
				*t.tag_owner = value.String
			}
