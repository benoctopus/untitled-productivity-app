...
func (tq *TagQuery) QueryOwner() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, false, tag.OwnerTable, tag.OwnerColumn),
		)
		fromU = sqlgraph.SetNeighbors(tq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (tq *TagQuery) loadTasks(ctx context.Context, query *TaskQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Task)) error {
	...
	for _, n := range neighbors {
		fk := n.tag_tasks
		if fk == nil {
			return fmt.Errorf(`foreign-key "tag_tasks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tag_tasks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (tq *TagQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *User)) error {
	...
	for i := range nodes {
		if nodes[i].tag_owner == nil {
			continue
		}
		fk := *nodes[i].tag_owner
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tag_owner" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
